...
- `quantwarden-backend-main/` — Python FastAPI + one Go microservices (stateless scan wrappers).
- `docker-compose.yml` (root) — builds and wires everything; see `README_DEPLOY.md`.
...
```
Ports: UI 3000, Mailpit 8025, worker 8088/8089, APIs 8000/8002/8010/8020/8085.
...
Backend (from `quantwarden-backend-main/`): `python3 start_monorepo_servers.py` (all services, auto-resolves port conflicts; `--setup` for interactive). Single service e.g. nmap-api: `cd nmap-api && python3 -m uvicorn main:app --host 0.0.0.0 --port 8010 --reload`. Go subfinder: `cd subfinder-api && ONEFORALL_API_URL=http://127.0.0.1:8002 SUBFINDER_API_ADDR=:8085 go run .`.
...
- **Shipped wake URL is wrong.** Root `.env.example` sets `SCAN_WORKER_WAKE_URL=http://quantwarden-worker:8088/wake`, but the worker only handles `POST /internal/wake` (`worker/src/index.ts`). With the shipped value, manual batches still queue but the worker stays in idle (~30min) mode instead of fast-polling. Set it to `...:8088/internal/wake`. `SCAN_WORKER_WAKE_SECRET` must be identical on app and worker; health is on `8089` (`/healthz`).
...
- **Auth is dual-mode.** Better Auth with `magicLink` + `organization` + `username`. Email sign-in is optional and gated on `SMTP_HOST` (unset → guest-only). Guests get a synthetic email `username@$GUEST_EMAIL_DOMAIN` so the email-keyed invite/inbox/membership machinery works unchanged. `/api/auth/methods` tells the UI which modes are enabled. No Google, no Resend. Guest password recovery is org-admin-only.
...
Worker tuning vars (defaults in `worker/src/config.ts`): active executor 1500ms, active scheduler 10000ms, idle 1800000ms, grace 60000ms, org limit 100. OpenSSL batching: `OPENSSL_API_PROBE_BATCH_SIZE=10`, `OPENSSL_API_TIMEOUT_SECONDS=3`, `OPENSSL_API_REQUEST_TIMEOUT_MS=15000`. Full list in `quantwarden-ui-main/.env.worker.example` and `worker/README.md`.
