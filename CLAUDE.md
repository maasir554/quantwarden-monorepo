...
- **Auth**: Better Auth (`src/lib/auth.ts`) with `organization`, `magicLink`, and `username` plugins. Two coexisting sign-in styles: **email** (OTP/magic-link, sent over SMTP via `src/lib/mailer.ts` — email is optional, gated on `SMTP_HOST`) and **guest** (username+password, `src/lib/guest-auth.ts`). Guests get a synthetic email `username@$GUEST_EMAIL_DOMAIN` so the email-keyed invite/membership/inbox machinery works unchanged. `/api/auth/methods` tells the UI which styles are enabled. Guest password recovery is org-admin-only (`/api/orgs/members/reset-password`). No Google, no Resend. The `user.username` columns are provisioned by `ensureGuestAuthSchema()` (runtime `ADD COLUMN IF NOT EXISTS`, like the scan-schedule tables). Org membership/permission checks live in `src/lib/org-scan-permissions.ts` and `src/lib/auth-session.ts`.
