...
This repository includes a Python-native REST API wrapper around the OneForAll core engine (no CLI subprocess calls).
...
```bash
curl -X POST http://localhost:8002/subdomains \
...
- API mode is collection-only and does not execute `massdns` paths.
- API scans keep the exact input scope. Example: input `pnb.bank.in` returns subdomains under `pnb.bank.in`, not all of `bank.in`.
- API input only requires one field: `domain`.
